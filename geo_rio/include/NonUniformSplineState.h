...
class NonUniformSplineState
{

  public:

    NonUniformSplineState() {};
              ...
              Eigen::Vector3d t0 = Eigen::Vector3d::Zero(), Eigen::Quaterniond q0 = Eigen::Quaterniond::Identity())
    {
        ...
        knots_time_ns.clear();
        knots_time_ns.push_back(-0.09 * 1e9);
        knots_time_ns.push_back(-0.06 * 1e9);
        knots_time_ns.push_back(-0.03 * 1e9);
    }
    ...
    void updateKnots(NonUniformSplineState* other)
    {
    ...
    void itpQuaternion(int64_t t_ns, Eigen::Quaterniond* q_out = nullptr,
                       Eigen::Vector3d* w_out = nullptr, Jacobian43* J_q = nullptr, Jacobian33* J_w = nullptr, Eigen::Matrix3d* unc = nullptr) const
    {
        double u;
        int64_t idx0;
        int idx_r;
        ...
        if (unc){
            ...
            *q_out = q_itps[3];
            ...
            // TODO : previous quaternion uncertainty add
            Eigen::Matrix3d unc_rot = Eigen::Matrix3d::Zero();
            // J_Rq = 2 [ -q_v ,  q_w I + [q_v]× ]
            Eigen::Matrix<double,3,4> J_Rq;
            J_Rq << -2 * q_out->x(),  2 * q_out->w(),  2 * q_out->z(), -2 * q_out->y(),
                -2 * q_out->y(), -2 * q_out->z(),  2 * q_out->w(),  2 * q_out->x(),
                -2 * q_out->z(),  2 * q_out->y(), -2 * q_out->x(),  2 * q_out->w();

            *unc = J_Rq * unc_quat * J_Rq.transpose();
  ...
  private:
    ...
    Eigen::aligned_deque<double> u_knots;
    std::vector<int64_t> knots_time_ns;
                        ...
                        const Eigen::aligned_deque<_KnotT>& knots, Jacobian* J = nullptr, Eigen::Matrix3d* unc = nullptr) const
    {
        ...
        if(unc) {
            ...
            for (int i = 0; i < size_J; i++) {
                // RCLCPP_INFO(rclcpp::get_logger("NonUniformSplineState"), "t cov idx : %d", idx_r - size_J + i);
...
// const Eigen::Matrix4d NonUniformSplineState::base_coefficients = NonUniformSplineState::computeBaseCoefficients();
// const Eigen::Matrix4d NonUniformSplineState::blending_matrix = NonUniformSplineState::computeBlendingMatrix();
// const Eigen::Matrix4d NonUniformSplineState::cumulative_blending_matrix = NonUniformSplineState::computeBlendingMatrix<true>();